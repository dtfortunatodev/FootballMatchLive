 ...
 */
public class IpCatchEntity implements Serializable{
    ...
    public boolean comparateUsersIp(IpCatchEntity ipCatchEntity){
        if(ipCatchEntity != null){
            ...
            // Contains Org
            if(ipCatchEntity.getOrg() != null && this.getOrg() != null && (ipCatchEntity.getOrg().contains(this.getOrg()) || this.getOrg().contains(ipCatchEntity.getOrg()))){
                return true;
            }
    ...
    public String exportJson() throws IOException {
        String json = ConnectionHelper.getObjectMapperInstance().writeValueAsString(this);
